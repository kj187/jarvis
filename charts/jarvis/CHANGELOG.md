...
- `auth.oidc.groupsClaim` and `auth.oidc.adminValue` values, rendered as `JARVIS_OIDC_GROUPS_CLAIM` and `JARVIS_OIDC_ADMIN_VALUE` in the ConfigMap, each only when set. They let SSO group membership (e.g. `cognito:groups`) grant the admin role without editing `extraEnv`. Empty by default, so an existing release renders unchanged.
- `config.retention.*` values (`days`, `eventsDays`, `claimsDays`, `silenceEventsDays`, `commentsDays`, `sweepInterval`), rendered as the corresponding `JARVIS_RETENTION_*` variables, each only when set. Controls the data-retention sweep; empty by default, matching the backend's own opt-in default of keeping history forever, so an existing release renders unchanged.
