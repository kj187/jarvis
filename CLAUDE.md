...
Jarvis is a web frontend for Prometheus Alertmanager. The Go 1.25+ backend (Echo v4) polls all configured Alertmanager clusters, stores every alert lifecycle in SQLite or PostgreSQL, keeps the current poll snapshot in an in-memory store, and pushes updates via WebSocket to the React 19 / TypeScript / Vite 8 frontend. The Go module path is `github.com/kj187/jarvis/backend`.
