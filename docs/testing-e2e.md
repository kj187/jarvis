...
| `settings.spec.ts` | H1–H4, H6, H7, H9–H15 (H5, H8 removed), H1b | All settings: timeFormat, defaultViewMode, resolvedPageSize, silenceDuration, defaultCreatorName, claimAnimation, reset defaults, persistence; brand footer (logo + version); `settings=open` survives reload and is removed on close without dropping other URL params (H12); Fast-Silence duration editor — edit/reset/validate and instance defaults from a mocked `GET /api/v1/settings` `global` (H13–H15) |
