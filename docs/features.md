...
2. Hovering, tapping, or pressing **Enter**/**Space** on the bell opens a popover (tabbing onto the bell does not open it, so moving through a page of cards stays quick): **Silence…** at the top opens the full pre-filled form (see [Silence from Alert](#silence-from-alert)); below it, **Fast-Silence for…** lists durations — by default `5m`, `10m`, `15m`, `30m`, `1h`, `4h`, `1d`, `1w` (configurable, see *Choosing the durations* below).
