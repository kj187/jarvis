...
test('B9 resolved mode uses flat list and page size controls', async ({ page, jarvis }) => {
  ...
  const topPager = page.getByTestId('resolved-pagination-top')
  const bottomPager = page.getByTestId('resolved-pagination-bottom')
  await expect(topPager).toHaveClass(/justify-end/)
  await expect(bottomPager).toHaveClass(/justify-end/)
  const topPageNavigation = page.getByRole('navigation', { name: 'Resolved alert pages', exact: true })
  await expect(topPageNavigation).toBeVisible()
  await expect(page.getByRole('navigation', { name: 'Resolved alert pages footer', exact: true })).toBeVisible()
  const pageSizeBox = await page.getByTestId('resolved-page-size').boundingBox()
  const pageNavigationBox = await topPageNavigation.boundingBox()
  expect(pageSizeBox).not.toBeNull()
  expect(pageNavigationBox).not.toBeNull()
  expect(pageSizeBox!.x + pageSizeBox!.width).toBeLessThan(pageNavigationBox!.x)
})
